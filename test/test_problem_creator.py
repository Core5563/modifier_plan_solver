...
def test_problem_creator_errors():
    """test the error of the problem creator"""
    # duplicate var name
    ...
    with pytest.raises(ValueError) as exception_info:
        ProblemCreator.create_problem([("x", True)], [("action1", [], [("x", True)]), ("action1", [], [("x", True)])], [])
    assert (ProblemCreator.ACTION_NAME_NOT_UNIQUE_ERROR_MESSAGE + " name: action1") in str(exception_info.value)

    # unknown precondition
    with pytest.raises(ValueError) as exception_info:
        ProblemCreator.create_problem([("x", True)], [("action1", ["y"], [("x", True)])], [])
    ...
    #empty effect list
    with pytest.raises(ValueError) as exception_info:
        ProblemCreator.create_problem([("x", True)], [("a1", [], [])], ["x"])
    assert (ProblemCreator.EMPTY_EFFECT_LIST_MESSAGE + " action name: a1") in str(exception_info.value)
