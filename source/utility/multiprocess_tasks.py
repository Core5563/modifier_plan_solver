...
from source.model.plan_modifiers.modifier_util import ground_solvable_problem, ground_problem
...
def ground_unsolvable_problem_multithread(problem: Problem, return_dict: dict[int, CompilerResult], error_dict: dict[int, str])-> None:
    """ground Problem with multithreading"""
    try:
        to_return = ground_problem(problem)
    except Exception:
        error_text = traceback.format_exc()
        error_dict[0] = error_text
        return
